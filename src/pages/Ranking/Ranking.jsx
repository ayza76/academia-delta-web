...
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, Trophy, Zap, Cpu } from 'lucide-react';

const Ranking = () => {
  const navigate = useNavigate();
  
  // DATOS DE RANKING FICTICIOS (Simulación de base de datos)
  const [rankingData] = useState([
    { Puesto: 1, Aka: 'B-Boy Shadow', 'Hip Hop': 18.5, Breaking: 19.2, Waacking: 15.0, Popping: 17.5, House: 16.8, Total: 87.0 },
    { Puesto: 2, Aka: 'Neon Spark', 'Hip Hop': 16.0, Breaking: 15.5, Waacking: 19.0, Popping: 14.2, House: 18.5, Total: 83.2 },
    { Puesto: 3, Aka: 'Cyber-V', 'Hip Hop': 15.5, Breaking: 14.0, Waacking: 14.5, Popping: 18.8, House: 15.2, Total: 78.0 },
    { Puesto: 4, Aka: 'Thunder-K', 'Hip Hop': 14.0, Breaking: 17.0, Waacking: 12.0, Popping: 15.5, House: 14.0, Total: 72.5 }
  ]);
  ...
  return (
    <div style={pageContainer}>
      ...
      <div style={contentWrapper}>
        ...
        <header style={headerStyle}>
          <div style={titleBadge}><Cpu size={14} style={{marginRight:'5px'}}/> Leaderboard Local</div>
          <h1 style={titleStyle}>
            RANKING OFICIAL <span style={neonText}>DELTA</span>
          </h1>
          <p style={subtitleStyle}>Métricas de rendimiento sincronizadas (Modo Previsualización).</p>
        </header>
...
const contentWrapper = { padding: '50px', position: 'relative', zIndex: 10, maxWidth: '1400px', margin: '0 auto' };
const glowBgTop = { position: 'fixed', top: '-20%', left: '-10%', width: '60vw', height: '60vw', background: 'radial-gradient(circle, rgba(99, 102, 241, 0.1) 0%, rgba(0,0,0,0) 60%)', zIndex: 0, pointerEvents: 'none' };
const glowBgBottom = { position: 'fixed', bottom: '-20%', right: '-10%', width: '50vw', height: '50vw', background: 'radial-gradient(circle, rgba(45, 212, 191, 0.08) 0%, rgba(0,0,0,0) 60%)', zIndex: 0, pointerEvents: 'none' };
const headerStyle = { marginBottom: '50px', marginTop: '10px' };
const titleBadge = { display: 'inline-flex', alignItems: 'center', padding: '4px 12px', backgroundColor: '#fbbf2420', color: '#fbbf24', borderRadius: '4px', fontSize: '12px', fontWeight: 'bold', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '15px', border: '1px solid #fbbf2440' };
const titleStyle = { fontSize: '42px', fontWeight: '900', margin: '0 0 10px 0', letterSpacing: '-1px', fontFamily: '"Impact", sans-serif' };
const neonText = { color: 'transparent', WebkitTextStroke: '2px #00ffcc', textShadow: '0 0 15px rgba(0, 255, 204, 0.5)', letterSpacing: '2px' };
const subtitleStyle = { color: '#94a3b8', fontSize: '16px', letterSpacing: '0.5px' };
const tableContainer = { backgroundColor: 'rgba(15, 23, 42, 0.6)', borderRadius: '20px', border: '1px solid #1e293b', boxShadow: '0 20px 50px rgba(0,0,0,0.7)', overflowX: 'auto', backdropFilter: 'blur(10px)' };
const thStyle = { padding: '20px', color: '#94a3b8', fontSize: '11px', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '2px', borderBottom: '1px solid #334155' };
const thStyleTotal = { ...thStyle, color: '#00ffcc' };
const tdStyle = { padding: '20px', fontSize: '14px', borderBottom: '1px solid rgba(30, 41, 59, 0.5)' };
const tdStyleNum = { ...tdStyle, color: '#cbd5e1', fontFamily: 'monospace', fontSize: '15px' };
const trStyle = { transition: 'all 0.2s ease', cursor: 'default' };
const topRankBadge = { backgroundColor: '#fbbf24', width: '30px', height: '30px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 15px rgba(251, 191, 36, 0.6)' };
const normalRank = { color: '#64748b', fontWeight: 'bold', fontSize: '16px' };
const backBtnStyle = { display: 'inline-flex', alignItems: 'center', gap: '8px', backgroundColor: 'rgba(30, 41, 59, 0.5)', color: '#94a3b8', border: '1px solid #334155', cursor: 'pointer', fontSize: '12px', marginBottom: '30px', padding: '10px 20px', borderRadius: '8px', textTransform: 'uppercase', letterSpacing: '1px' };
const badgeStyle = (puntos) => ({ backgroundColor: puntos >= 80 ? 'rgba(0, 255, 204, 0.1)' : 'rgba(99, 102, 241, 0.1)', color: puntos >= 80 ? '#00ffcc' : '#818cf8', border: `1px solid ${puntos >= 80 ? 'rgba(0, 255, 204, 0.3)' : 'rgba(99, 102, 241, 0.3)'}`, padding: '6px 14px', borderRadius: '4px', fontSize: '11px', fontWeight: '900', letterSpacing: '1px', whiteSpace: 'nowrap' });
