...
import { useNavigate } from 'react-router-dom';
import { UserPlus, Edit, Trash2, X, Save, Calendar, Cpu } from 'lucide-react';

const Miembros = () => {
  const navigate = useNavigate();
  
  // DATOS DE PRUEBA FICTICIOS (Simulación de base de datos)
  const [miembros, setMiembros] = useState([
    { IdIntegrante: 101, Nombre: 'Carlos Espinoza', Aka: 'B-Boy Shadow', Edad: 24, FechaIngreso: '2025-01-15' },
    { IdIntegrante: 102, Nombre: 'Mariana Valdivia', Aka: 'Spark', Edad: 21, FechaIngreso: '2025-02-10' },
    { IdIntegrante: 103, Nombre: 'Ricardo Mendoza', Aka: 'Thunder', Edad: 26, FechaIngreso: '2024-11-20' }
  ]);
  ...
  const handleDelete = (id, aka) => {
    if (window.confirm(`⚠️ MODO DISEÑO: ¿Eliminar visualmente a ${aka}?`)) {
      setMiembros(miembros.filter(m => m.IdIntegrante !== id));
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (editMode) {
      // Simular Update
      setMiembros(miembros.map(m => m.IdIntegrante === selectedId ? { ...formData, IdIntegrante: selectedId } : m));
    } else {
      // Simular Create
      const newId = Math.floor(Math.random() * 1000) + 200;
      setMiembros([...miembros, { ...formData, IdIntegrante: newId }]);
    }
    setIsModalOpen(false);
  };
  ...
  return (
    <div style={pageContainer}>
      ...
      <div style={contentWrapper}>
        <header style={headerStyle}>
          <div>
            <div style={titleBadge}><Cpu size={14}/> Terminal Local</div>
            <h1 style={titleStyle}>GESTIÓN DE <span style={neonText}>CREW</span></h1>
            <p style={subtitleStyle}>Prototipo de interfaz para control de registros de integrantes.</p>
          </div>
          <button onClick={handleOpenAdd} style={addBtnStyle}>
            <UserPlus size={20} /> NUEVO INGRESO
          </button>
        ...
        {isModalOpen && (
          <div style={modalOverlay}>
            <div style={modalContent}>
              <div style={modalHeader}>
                <h2 style={{display: 'flex', alignItems: 'center', gap: '10px', color: '#fff', fontSize: '20px'}}>
                  {editMode ? <Edit size={24} color="#00ffcc"/> : <UserPlus size={24} color="#00ffcc"/>} 
                  {editMode ? `EDITANDO NODO [ID:${selectedId}]` : 'NUEVO REGISTRO'}
                </h2>
                <X onClick={() => setIsModalOpen(false)} style={closeBtn} />
              </div>
              ...
              <form onSubmit={handleSubmit} style={{display: 'flex', flexDirection: 'column', gap: '20px'}}>
                <div style={inputGroup}>
                  <label style={labelStyle}>NOMBRE COMPLETO</label>
                  <input style={inputStyle} value={formData.Nombre} onChange={(e) => setFormData({...formData, Nombre: e.target.value})} placeholder="Ej: Roberto Pérez" required />
                </div>
                ...
                <div style={{display: 'flex', gap: '15px'}}>
                  <div style={{...inputGroup, flex: 1}}>
                    <label style={labelStyle}>EDAD</label>
                    <input style={inputStyle} type="number" value={formData.Edad} onChange={(e) => setFormData({...formData, Edad: e.target.value})} required />
                  </div>
                ...
                <button type="submit" style={saveBtnStyle}>
                  <Save size={18} /> {editMode ? 'ACTUALIZAR VISTA' : 'AÑADIR A LA LISTA'}
                </button>
              </form>
            </div>
          </div>
            ...
            return (
              <div 
                ...
                style={{
                  ...cardStyle,
                  borderColor: isHovered ? '#00ffcc' : '#1e293b',
                  boxShadow: isHovered ? '0 10px 30px rgba(0,255,204,0.15)' : '0 5px 15px rgba(0,0,0,0.5)',
                  transform: isHovered ? 'translateY(-5px)' : 'translateY(0)'
                }}
                onMouseEnter={() => setHoveredCard(m.IdIntegrante)}
                onMouseLeave={() => setHoveredCard(null)}
              >
                ...
                <div style={idTag}>REF-{m.IdIntegrante}</div>
              </div>
            )
          })}
        </div>
...
const neonText = { color: 'transparent', WebkitTextStroke: '2px #f43f5e', textShadow: '0 0 15px rgba(244, 63, 94, 0.5)', letterSpacing: '2px' };
const subtitleStyle = { color: '#94a3b8', fontSize: '16px', letterSpacing: '0.5px' };
const addBtnStyle = { display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: '#00ffcc', border: 'none', color: '#000', padding: '14px 28px', borderRadius: '12px', fontWeight: '900', cursor: 'pointer', letterSpacing: '1px', boxShadow: '0 0 15px rgba(0,255,204,0.4)' };
const modalOverlay = { position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(5, 5, 10, 0.85)', backdropFilter: 'blur(8px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 };
const modalContent = { backgroundColor: '#0f172a', padding: '40px', borderRadius: '20px', width: '450px', border: '1px solid #00ffcc' };
const modalHeader = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', borderBottom: '1px solid #1e293b', paddingBottom: '15px' };
const closeBtn = { cursor: 'pointer', color: '#94a3b8' };
const inputGroup = { display: 'flex', flexDirection: 'column', gap: '8px' };
const labelStyle = { fontSize: '11px', color: '#00ffcc', fontWeight: 'bold', letterSpacing: '1px' };
const inputStyle = { padding: '14px', borderRadius: '8px', border: '1px solid #334155', backgroundColor: 'rgba(5, 5, 10, 0.5)', color: '#fff', fontSize: '15px', outline: 'none', colorScheme: 'dark', fontFamily: 'monospace' };
const saveBtnStyle = { padding: '16px', borderRadius: '8px', border: 'none', backgroundColor: '#00ffcc', color: '#000', fontWeight: '900', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginTop: '10px', letterSpacing: '1px', textTransform: 'uppercase' };
const cardsGrid = { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '30px' };
const cardStyle = { backgroundColor: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(5px)', borderRadius: '16px', padding: '30px', border: '1px solid #1e293b', textAlign: 'center', transition: 'all 0.3s ease', position: 'relative', overflow: 'hidden' };
const avatarStyle = { width: '80px', height: '80px', backgroundColor: '#05050a', border: '2px solid #00ffcc', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px auto', fontSize: '32px', fontWeight: '900', color: '#00ffcc' };
const cardAka = { fontSize: '24px', fontWeight: '900', margin: '0 0 5px 0', letterSpacing: '1px', textTransform: 'uppercase' };
const cardNombre = { color: '#94a3b8', fontSize: '13px', margin: '0 0 20px 0' };
const infoBadgeContainer = { display: 'flex', justifyContent: 'center', gap: '10px', marginBottom: '25px' };
const infoItem = { backgroundColor: '#1e293b', padding: '6px 12px', borderRadius: '6px', fontSize: '11px', color: '#cbd5e1', display: 'flex', alignItems: 'center', gap: '6px', fontWeight: 'bold' };
const actionGroup = { display: 'flex', justifyContent: 'center', gap: '15px' };
const iconBtn = (c) => ({ backgroundColor: 'transparent', color: c, border: `1px solid ${c}40`, padding: '10px 15px', borderRadius: '8px', cursor: 'pointer' });
const idTag = { position: 'absolute', top: '15px', right: '15px', fontSize: '10px', color: '#475569', fontWeight: 'bold', fontFamily: 'monospace' };
