...
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, Save, Plus, Trash2, Activity, Cpu } from 'lucide-react';

const Calificaciones = () => {
  const navigate = useNavigate();
  
  // Datos estáticos para simular la base de datos
  const miembrosFicticios = [
    { IdIntegrante: 1, Aka: 'B-Boy Ghost' },
    { IdIntegrante: 2, Aka: 'Wonder Woman' },
    { IdIntegrante: 3, Aka: 'Nightmare' },
    { IdIntegrante: 4, Aka: 'Street Soul' }
  ];
  ...
  const handleSubmit = (e) => {
    ...
    console.log("SIMULACIÓN DE ENVÍO:", { idIntegrante, fecha, filasEvaluacion });
    alert("✔️ MODO DISEÑO: Datos procesados localmente con éxito.");
  };
      ...
      <div style={contentWrapper}>
        ...
        <header style={headerStyle}>
          <div>
            <div style={titleBadge}><Cpu size={14}/> Terminal de Datos</div>
            <h1 style={titleStyle}>EVALUACIÓN DE <span style={neonText}>DESEMPEÑO</span></h1>
            <p style={subtitleStyle}>Prototipo de interfaz para ingreso de métricas de rendimiento.</p>
          </div>
        </header>
        ...
        <form onSubmit={handleSubmit} style={formCardStyle}>
          ...
          <div style={masterGrid}>
            <div style={inputContainer}>
              ...
              >
                <option value="">Seleccionar AKA...</option>
                {miembrosFicticios.map(m => (
                  <option key={m.IdIntegrante} value={m.IdIntegrante}>{m.Aka}</option>
                ))}
              </select>
            </div>
            <div style={inputContainerDate}>
          ...
          <button type="submit" style={saveBtnStyle}>
            <Save size={20} /> SIMULAR TRANSMISIÓN DE DATOS
          </button>
        </form>
      </div>
    </div>
...
const subtitleStyle = { color: '#94a3b8', fontSize: '16px', letterSpacing: '0.5px' };
const backBtnStyle = { display: 'inline-flex', alignItems: 'center', gap: '8px', backgroundColor: 'rgba(30, 41, 59, 0.5)', color: '#94a3b8', border: '1px solid #334155', cursor: 'pointer', fontSize: '12px', marginBottom: '30px', padding: '10px 20px', borderRadius: '8px', textTransform: 'uppercase', letterSpacing: '1px', transition: 'all 0.3s' };
const formCardStyle = { backgroundColor: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(10px)', padding: '40px', borderRadius: '20px', border: '1px solid #334155', boxShadow: '0 20px 50px rgba(0,0,0,0.5), inset 0 0 20px rgba(129, 140, 248, 0.05)', width: '100%' };
const masterGrid = { display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px', marginBottom: '30px' };
const inputContainer = { display: 'flex', flexDirection: 'column', gap: '8px' };
const inputContainerDate = { display: 'flex', flexDirection: 'column', gap: '8px' };
const labelStyle = { fontSize: '11px', color: '#818cf8', fontWeight: 'bold', letterSpacing: '1px' };
const inputStyle = { padding: '14px', borderRadius: '8px', border: '1px solid #334155', backgroundColor: 'rgba(5, 5, 10, 0.5)', color: '#fff', fontSize: '14px', outline: 'none', colorScheme: 'dark', fontFamily: 'monospace' };
const notaInputStyle = { ...inputStyle, width: '100px', textAlign: 'center', fontSize: '16px', fontWeight: 'bold', color: '#00ffcc', borderColor: '#818cf8' };
const selectStyle = { ...inputStyle, cursor: 'pointer', fontWeight: 'bold' };
const divider = { borderTop: '1px dashed #334155', paddingTop: '30px' };
const detailHeader = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' };
const filaEstiloContainer = { display: 'flex', gap: '15px', marginBottom: '20px', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.2)', padding: '15px', borderRadius: '12px', borderLeft: '3px solid #818cf8' };
const addFilaBtn = { backgroundColor: 'transparent', color: '#818cf8', border: '1px solid #818cf8', padding: '10px 16px', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', fontWeight: 'bold', letterSpacing: '1px' };
const deleteBtn = { color: '#f43f5e', background: 'rgba(244, 63, 94, 0.1)', border: '1px solid rgba(244, 63, 94, 0.3)', padding: '12px', borderRadius: '8px', cursor: 'pointer' };
const saveBtnStyle = { width: '100%', padding: '18px', borderRadius: '12px', border: 'none', backgroundColor: '#818cf8', color: '#000', fontWeight: '900', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginTop: '40px', letterSpacing: '2px', textTransform: 'uppercase', boxShadow: '0 0 20px rgba(129, 140, 248, 0.4)' };
