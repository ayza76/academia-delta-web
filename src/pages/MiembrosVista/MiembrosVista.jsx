...
import { useNavigate } from 'react-router-dom';
import { ArrowLeft, Calendar, User, Search, Zap, Crosshair, Cpu } from 'lucide-react';

const MiembrosVista = () => {
  const navigate = useNavigate();
  
  // DATOS DE INTELIGENCIA FICTICIOS (Simulación de base de datos)
  const [miembros] = useState([
    { IdIntegrante: 101, Aka: 'Ghost', Nombre: 'Erick Harrison', Edad: 24, FechaIngreso: '2024-05-12' },
    { IdIntegrante: 102, Aka: 'Cyber-V', Nombre: 'Valeria Sokolov', Edad: 22, FechaIngreso: '2024-08-19' },
    { IdIntegrante: 103, Aka: 'Neon-Rush', Nombre: 'Marcus Thorne', Edad: 27, FechaIngreso: '2023-11-02' },
    { IdIntegrante: 104, Aka: 'Oracle', Nombre: 'Elena Vance', Edad: 25, FechaIngreso: '2025-01-10' }
  ]);
  ...
  return (
    <div style={pageContainer}>
      ...
      <div style={contentWrapper}>
        ...
        <header style={headerSection}>
          <div style={titleBadge}><Cpu size={14}/> Terminal de Consulta</div>
          <h1 style={titleStyle}>ROSTER DE <span style={neonText}>AGENTES</span></h1>
          <p style={subtitleStyle}>Visualización de inteligencia y perfiles de la tripulación DELTA.</p>
          ...
          <div style={searchWrapper}>
            <div style={searchContainer}>
              ...
              <input 
                style={searchInput} 
                placeholder="ESCRIBE AKA O NOMBRE PARA FILTRAR..." 
                value={filtro}
                onChange={(e) => setFiltro(e.target.value)}
              />
            </div>
            {filtro && (
              <div style={searchResultText}>
                {miembrosFiltrados.length} {miembrosFiltrados.length === 1 ? 'REGISTRO' : 'REGISTROS'} ENCONTRADOS
              </div>
            )}
          </div>
        </header>
            ...
            return (
              <div 
                ...
                style={{
                  ...memberCard,
                  borderColor: isHovered ? '#f43f5e' : '#1e293b',
                  boxShadow: isHovered ? '0 15px 35px rgba(244, 63, 94, 0.2)' : '0 10px 30px rgba(0,0,0,0.5)',
              ...
              >
                <div style={{...cardHeader, borderBottom: isHovered ? '1px solid #f43f5e40' : '1px solid #1e293b'}}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '5px' }}>
                    <div style={idBadge}>ID // {m.IdIntegrante.toString().padStart(4, '0')}</div>
                    {isHovered && <Crosshair size={14} color="#f43f5e" />}
                  </div>
                </div>
                ...
                <div style={cardFooter}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{...statusDot, backgroundColor: '#22c55e'}}></span> 
                    <span style={{ color: isHovered ? '#fff' : '#64748b' }}>ACTIVO</span>
                  </div>
                  {isHovered && <Zap size={14} color="#fbbf24" />}
                </div>
              </div>
            );
          })}
...
const glowBgBottom = { position: 'fixed', bottom: '-20%', right: '-10%', width: '50vw', height: '50vw', background: 'radial-gradient(circle, rgba(99, 102, 241, 0.05) 0%, rgba(0,0,0,0) 60%)', zIndex: 0, pointerEvents: 'none' };
const headerSection = { marginBottom: '50px', marginTop: '10px' };
const backBtn = { display: 'inline-flex', alignItems: 'center', gap: '8px', backgroundColor: 'rgba(30, 41, 59, 0.5)', color: '#94a3b8', border: '1px solid #334155', cursor: 'pointer', fontSize: '12px', marginBottom: '30px', padding: '10px 20px', borderRadius: '8px', textTransform: 'uppercase', letterSpacing: '1px' };
const titleBadge = { display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '4px 12px', backgroundColor: '#f43f5e20', color: '#f43f5e', borderRadius: '4px', fontSize: '12px', fontWeight: 'bold', letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '15px', border: '1px solid #f43f5e40' };
const titleStyle = { fontSize: '42px', fontWeight: '900', margin: '0 0 10px 0', letterSpacing: '-1px', fontFamily: '"Impact", "Arial Black", sans-serif' };
const neonText = { color: 'transparent', WebkitTextStroke: '2px #f43f5e', textShadow: '0 0 15px rgba(244, 63, 94, 0.6)', letterSpacing: '2px' };
const subtitleStyle = { color: '#94a3b8', fontSize: '16px', letterSpacing: '0.5px', marginBottom: '35px' };
const searchWrapper = { display: 'flex', flexDirection: 'column', gap: '10px' };
const searchContainer = { display: 'flex', alignItems: 'center', gap: '15px', backgroundColor: 'rgba(15, 23, 42, 0.8)', padding: '18px 25px', borderRadius: '16px', border: '1px solid #f43f5e50', maxWidth: '500px', backdropFilter: 'blur(10px)' };
const searchInput = { background: 'none', border: 'none', color: '#fff', outline: 'none', width: '100%', fontSize: '16px', fontFamily: 'monospace' };
const searchResultText = { fontSize: '11px', color: '#f43f5e', fontFamily: 'monospace', marginLeft: '10px', fontWeight: 'bold' };
const gridContainer = { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '30px' };
const memberCard = { backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(10px)', borderRadius: '24px', border: '1px solid #1e293b', overflow: 'hidden', transition: 'all 0.4s ease', position: 'relative' };
const cardHeader = { padding: '25px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', background: 'linear-gradient(135deg, rgba(30, 41, 59, 0.5) 0%, rgba(15, 23, 42, 0.8) 100%)' };
const profileCircle = { width: '60px', height: '60px', borderRadius: '16px', backgroundColor: '#05050a', border: '2px solid #f43f5e', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', fontWeight: '900', color: '#f43f5e' };
const idBadge = { fontSize: '11px', color: '#cbd5e1', backgroundColor: '#0f172a', padding: '6px 12px', borderRadius: '6px', fontFamily: 'monospace', border: '1px solid #334155' };
const cardBody = { padding: '25px' };
const akaTitle = { fontSize: '26px', fontWeight: '900', margin: '0 0 5px 0', letterSpacing: '1px', textTransform: 'uppercase' };
const fullName = { color: '#64748b', fontSize: '13px', marginBottom: '25px' };
const infoGrid = { display: 'flex', flexDirection: 'column', gap: '15px' };
const infoItem = { display: 'flex', alignItems: 'center', gap: '15px' };
const infoIconBox = { backgroundColor: '#f43f5e15', padding: '8px', borderRadius: '8px', border: '1px solid #f43f5e30' };
const infoTextColumn = { display: 'flex', flexDirection: 'column', gap: '2px' };
const infoLabel = { fontSize: '10px', color: '#64748b', fontWeight: 'bold' };
const infoValue = { fontSize: '14px', color: '#f8fafc', fontWeight: 'bold', fontFamily: 'monospace' };
const cardFooter = { padding: '15px 25px', borderTop: '1px solid rgba(30, 41, 59, 0.5)', fontSize: '11px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' };
const statusDot = { width: '8px', height: '8px', borderRadius: '50%', display: 'inline-block' };
